...
def main(args):
    ...
    if 'iperf' in topo_yaml:
        ...
        for _client in _iperf['clients']:
            ...
            CMDS_CREATE.append("docker exec {0} iperf3 -c {1} -p {2} -b {3} -n {4}".format(_client['client'], _target, _port, _brate, _total))
            CMDS_START.append("docker exec {0} iperf3 -c {1} -p {2} -b {3} -n {4}".format(_client['client'], _target, _port, _brate, _total))
